...
function terminology() { const airline = $('airlineChoice').value; if (airline === 'delta') return { single: 'Rotation', plural: 'rotations', title: 'Recommended rotations' }; if (airline === 'southwest') return { single: 'Line', plural: 'lines', title: 'Recommended lines' }; return { single: airline === 'american' ? 'Sequence' : 'Pairing', plural: airline === 'american' ? 'sequences' : 'pairings', title: airline === 'american' ? 'Recommended sequences' : 'Recommended pairings' }; }
...
function render() {
  ...
  shown.forEach((item, index) => {
    ...
    card.innerHTML = `<div class="rank-badge">${index + 1}</div><div class="result-main"><div class="result-top"><div><span class="item-label">${esc(item.display_label || term.single)}</span><h3>${esc(item.pairing)}</h3></div><span class="match-pill ${cls}">${label}</span></div><div class="metric-strip"><div><span>Credit</span><strong>${esc(item.credit || '—')}</strong></div><div><span>TAFB</span><strong>${esc(item.tafb || '—')}</strong></div><div><span>Legs by duty day</span><strong>${esc(legs)}</strong></div><div><span>First / Last</span><strong>${esc(item.first_day_legs ?? '—')} / ${esc(item.last_day_legs ?? '—')}</strong></div></div><div class="status-row"><span class="status ${recCls}">${esc(rec)}</span><span class="status neutral">Overnights: ${esc(layovers)}</span>${conflicts.length ? `<span class="status low">${conflicts.length} conflict${conflicts.length > 1 ? 's' : ''}</span>` : '<span class="status excellent">No conflicts</span>'}</div><details><summary>Why this fits your life</summary><div class="detail-grid"><div><h4>Why it matched</h4><ul>${(reasons.length ? reasons : ['No strong preference signals were detected.']).map(x => `<li>${esc(x)}</li>`).join('')}</ul></div><div><h4>Summary</h4><p><strong>Layovers:</strong> ${esc((item.cities || []).join(', ') || 'None')}</p>${item.equipment_codes?.length ? `<p><strong>Equipment:</strong> ${esc(item.equipment_codes.join(', '))}${item.equipment_mapping_status === 'raw_unmapped' ? ' (AA codes, mapping pending)' : ''}</p>` : ''}<p><strong>Deadheads:</strong> ${esc(item.deadheads || 0)}</p><p><strong>Redeyes:</strong> ${esc(item.redeye || 'none')}</p><p><strong>Soft credit:</strong> ${esc(soft)}</p><p><strong>Conflicts:</strong> ${esc(conflicts.join('; ') || 'None')}</p></div></div><h4>Timeline and duty legs</h4>${timeline(item)}<details class="operating-cities"><summary>All operating cities</summary><p>${esc((item.touched_cities || []).join(', ') || 'Not available')}</p></details><details class="original-display"><summary>Original airline formatting</summary><pre>${esc(item.original_display || 'Not available')}</pre></details></details></div>`;
    wrap.appendChild(card);
  });
  if (!shown.length) wrap.innerHTML = '<div class="empty-state">Your ranked results will appear here.</div>';
  if (allResults.length) { const top = allResults[0], [topRating] = rating(top), [topRecovery] = recovery(top); $('summary').textContent = `${allResults.length} ${term.plural} analyzed · showing ${shown.length}. Ratings reflect your selected preferences.`; $('snapshotMatch').textContent = topRating; $('snapshotCredit').textContent = top.credit || '—'; $('snapshotLength').textContent = top.duty_legs && top.duty_legs.length ? `${top.duty_legs.length}-day` : '—'; $('snapshotRecovery').textContent = topRecovery.replace(' recovery', ''); }
}
